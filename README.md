...
На строке отображаются: текущий распознанный объект, уверенность (с цветовой
индикацией: зелёный ≥80%, жёлтый ≥50%, красный \<50%), полоса уверенности,
FPS, количество детекций и время работы. При смене объекта печатается
отдельная строка с переходом.

В режиме `stdout` (`-o stdout`) выводятся табулированные строки для передачи
в другие программы (grep, awk, логирование).

Лог-файл `lobe_server.log` всегда записывается рядом с программой.
