...
В режиме `user` (по умолчанию) сервер показывает компактную строку состояния,
которая обновляется каждые 0.2 с:

<p align="center">
  <img src="screenshots-light.png" alt="Светлая тема" width="520"/>
  <br/>
  <img src="screenshots-dark.png" alt="Тёмная тема" width="520"/>
</p>

Панель состоит из 4 строк: заголовок с меткой, уверенностью, цветовой
полосой, FPS, номером детекции и временем с последнего изменения; сплошная
полоса уверенности; статистика топ-4 меток с мини-полосами и счётчиками;
нижняя граница. Бейдж соединения показывает статус (connecting/reconnecting).

Панель обновляется каждые 0.2 с. При смене метки печатается отдельная строка
с переходом.
