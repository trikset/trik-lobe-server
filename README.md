...
релизу прикреплены исполняемые файлы для Windows, Linux и macOS, а также
`settings.ini`. Релиз сначала создаётся как черновик, а публикуется после
проверки сопровождающим.

Имена файлов содержат версию релиза, например
`TRIKLobeServer-v26.08.03-Windows.exe` или
`TRIKLobeServer-v26.08.03-Linux.tar.gz`. Внутри архива Linux/macOS лежит
исполняемый файл с версией в имени (`TRIKLobeServer-v26.08.03.bin`).
